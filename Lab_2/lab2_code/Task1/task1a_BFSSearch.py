...
class BFSSearch:
    ...
    def search(self,map, start, goal):
        ...
        def get_neighbors2(currentNode):
            currentNodeX = currentNode[0]
            currentNodeY = currentNode[1]
            neighbours= []
            if ([currentNodeX + 1 , currentNodeY] not in came_from.values()) and currentNodeX + 1 not in {-1,60}:
                    if map[currentNodeX + 1][currentNodeY] is not (-1):
                        neighbours.append([currentNodeX + 1, currentNodeY ])
                        
            if ([currentNodeX , currentNodeY + 1] not in came_from.values())  and currentNodeY + 1 not in {-1,60}:
                    if map[currentNodeX][currentNodeY + 1] is not (-1):
                        neighbours.append([currentNodeX, currentNodeY + 1])
