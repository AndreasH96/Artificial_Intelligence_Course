...
class AStarAgentCostum:
    ...
    def calcObstacleCost(self, position):
        if position[1] < self.boardInfo["TopY"] and position[1] > self.boardInfo["BottomY"]:
            if position[0] < self.boardInfo["MiddleX"]:
                return 30 - (self.boardInfo["MiddleX"] - position[0])
            return 0
        else: 
            return 0

    def calculateCost(self,newNode):
        return self.euclidianDistance(newNode.coordinates, self.goalNode.coordinates) + newNode.depth - self.calcObstacleCost(newNode.coordinates)
